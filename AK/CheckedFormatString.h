...
consteval auto count_fmt_params(char const (&fmt)[N])
{
    struct {
        ...
        std::array<size_t, 128> used_arguments { 0 };
        ...
        std::array<size_t, 4> last_format_specifier_start { 0 };
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        case '{':
            ...
            if (result.total_used_last_format_specifier_start_count >= result.last_format_specifier_start.size() - 1)
                throw "Format-String Checker internal error: Format specifier nested too deep";
        ...
        case '}':
            ...
            if (result.unclosed_braces) {
                ...
                if (result.total_used_last_format_specifier_start_count == 0)
                    throw ("Format-String Checker internal error: Expected location information");
                ...
                if (result.total_used_argument_count >= result.used_arguments.size())
                    throw ("Format-String Checker internal error: Too many format arguments in format string");
...
private:
    ...
    consteval static bool check_format_parameter_consistency(char const (&fmt)[N])
    {
        ...
        if (check.unclosed_braces != 0)
            throw ("Extra unclosed braces in format string");
        if (check.extra_closed_braces != 0)
            throw ("Extra closing braces in format string");

        {
            auto begin = check.used_arguments.begin();
            auto end = check.used_arguments.begin() + check.total_used_argument_count;
            auto has_all_referenced_arguments = !std::any_of(begin, end, [](auto& entry) { return entry >= param_count; });
            if (!has_all_referenced_arguments)
                throw ("Format string references nonexistent parameter");
        }

        if (!check.has_explicit_argument_references && check.total_used_argument_count != param_count)
            throw ("Format string does not reference all passed parameters");
        ...
        if (check.has_explicit_argument_references) {
            ...
            auto references_all_arguments = std::all_of(
                all_parameters,
                [&](auto& entry) {
                    return contains(
...
template<typename... Args>
using CheckedFormatString = Format::Detail::CheckedFormatString<std::type_identity<Args>...>;
